          ...
          <div className="container mx-auto px-6 lg:px-12">
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-8">
              <div className="max-w-3xl">
                ...
                <RevealWords 
                  text={product.name} 
                  className="text-6xl md:text-8xl font-serif text-black dark:text-white leading-[0.95] mb-2" 
                />
              </div>
              <FadeIn delay={0.4}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
            ...
            <div className="lg:col-span-5 flex flex-col space-y-12">
              ...
              <div className="grid grid-cols-2 gap-8 py-8 border-y border-zinc-100 dark:border-zinc-900">
                {[
                  { icon: Droplets, label: "Color", value: product.specs.color },
                  { icon: Package, label: "Packaging", value: product.specs.packaging },
                  { icon: Layers, label: "Coverage", value: product.specs.coverage },
                  { icon: Zap, label: "Pot Life", value: product.specs.potLife }
                ].map((spec, i) => (
                  <FadeIn key={i} direction="left" delay={0.3 + i * 0.1}>
                    <div className="space-y-1">
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest flex items-center gap-2">
                        <spec.icon className="w-3 h-3 text-[#00A550]" /> {spec.label}
                      </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                ...
                {PRODUCT_DATA[0].items.filter(i => i.id !== productId).slice(0, 3).map((item, idx) => (
                    <FadeIn key={idx} delay={idx * 0.1}>
                      <Link to={`/products/${item.id}`} className="group space-y-6">
